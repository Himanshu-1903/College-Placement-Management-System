...
function Signup() {
  document.title = 'IIIT Allahabad Placement Portal | Student Sign Up';
  ...
  const { first_name, number, email, password, department } = formData;
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData?.email && !formData?.first_name && !formData?.number && !formData?.password && !formData?.department)
      return setError({ email: 'Email Required!', first_name: 'Name Required!', number: 'Number Required!', password: 'Password Required!', department: 'Department Required!' })
    if (!formData?.email || !formData?.first_name || !formData?.number || !formData?.password || !formData?.department) {
      let email, first_name, number, password, department;
      ...
      if (!formData?.department) department = 'Department Required!';
      setError({ email: email, first_name: first_name, number: number, password: password, department: department })
      return;
    }
      ...
      <div className="flex justify-center items-center py-2 min-h-screen bg-gradient-to-r from-red-400 from-10% via-pink-300 via-40% to-purple-300 to-100% ">
        <form className="form-signin flex justify-center items-center flex-col gap-3 backdrop-blur-md bg-white/30 border border-white/20 rounded-lg shadow shadow-red-400 p-8 w-1/3 max-lg:w-2/3 max-md:w-3/4 max-[400px]:w-4/5" onSubmit={handleSubmit}>
          <div className='flex justify-center items-center flex-col'>
            <img className="mb-4 rounded-xl shadow w-30 h-28 lg:w-40 lg:h-40" src="/iiita.jpg" alt="Logo Image" />
          ...
          <div className="w-full">
            ...
            <select id="inputDepartment" className="form-control ml-1" name='department' value={department} onChange={handleChange}>
          ...
          <p className="text-muted text-center text-gray-400">© IIIT Allahabad Placement Portal {new Date().getFullYear()}</p>
        </form>
      </div>
    </>
  )
