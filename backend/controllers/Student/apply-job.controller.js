...
const AppliedToJob = async (req, res) => {
  try {
    ...
    if (user?.studentProfile?.appliedJobs?.some(j => j.jobId == req.params.jobId)) return res.status(400).json({ msg: "Already Applied!" });

    if (!user?.studentProfile?.resume) return res.status(400).json({ msg: 'Please Upload Resume First, Under "Placements" > "Placement Profile"' });
